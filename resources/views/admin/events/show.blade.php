...
@section('content')
    ...
    <h1 class="mb-4 text-primary"
        style="font-family: 'Roboto', sans-serif; font-weight: 700; color: #004085; text-align: center; margin-bottom: 20px;">
        Detail Event</h1>

    <div class="d-flex justify-content-between mb-4">
        <!-- Tombol Kembali di kiri -->
        <a href="{{ route('events.index') }}" class="btn btn-secondary btn-sm mt-3"
            style="padding: 8px 16px; background-color: #6c757d; color: white; font-size: 1rem; border-radius: 50px;">
        ...
        <div class="d-flex gap-2">
            @can('create_peserta')
                <a href="{{ route('events.members.create', ['event_id' => $event->id]) }}" class="btn btn-primary"
                    style="padding: 12px 20px; background-color: #007bff; color: white; font-size: 1rem; border-radius: 50px; transition: all 0.3s ease; display: inline-flex; align-items: center;">
                    <img width="20" height="20" src="https://img.icons8.com/ultraviolet/50/add-user-group-woman-man.png"
                        alt="add-user-group-woman-man" style="margin-right: 8px;" /> Tambah Peserta
                </a>
            @endcan
            @can('export_pdf')
                <a href="{{ route('events.export_pdf', ['event_id' => $event->id]) }}" class="btn btn-success"
                    style="padding: 12px 20px; background-color: #28a745; color: white; font-size: 1rem; border-radius: 50px; transition: all 0.3s ease; display: inline-flex; align-items: center;">
                    <img width="20" height="20" src="https://img.icons8.com/arcade/64/ms-word.png" alt="ms-word"
                        style="margin-right: 8px;" /> Export PDF
                </a>
            @endcan
        </div>
    </div>
    ...
    <div class="table-responsive mb-4"
        style="background-color: #fff; padding: 30px; border-radius: 8px; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.1);">
    ...
    <div class="mt-4">
        ...
        @else
            <div class="table-responsive"
                style="background-color: #fff; padding: 20px; border-radius: 8px; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.1);">
                <table class="table table-bordered table-striped">
                    <thead>
                        <tr>
                            <th>No.</th>
                                ...
                                <td>
                                    @can('delete_peserta')
                                        <button type="button" class="btn btn-danger btn-sm"
                                            style="padding: 6px 12px; background-color: #dc3545; color: white; border-radius: 5px; transition: all 0.3s ease;"
            ...
            <form id="deleteForm" action="" method="POST">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            <label for="reason" class="form-label" style="font-weight: 600; color: #333;">Alasan
                                Penghapusan</label>
                            <textarea name="reason" id="reason" class="form-control" rows="3"
                                style="border-radius: 8px; border: 1px solid #ccc;" required></textarea>
                        </div>
                    </div>
